...
#### 접근 아이디어
- N번째 손님은 낮은 층을 우선 배정받으며 같은 층이면 호실 번호가 낮은 방을 배정받는다
- N을 층 수 H로 나누면 몫은 호실 번호, 나머지는 층 번호에 대응된다
- 나머지가 0인 경우 H층이며 호실 번호는 몫 그대로 사용한다
- 호실 번호가 한 자리일 경우 앞에 `0`을 붙여 두 자리로 만든다

#### 알고리즘 판단 근거
- 단순 수학 연산으로 해결 가능한 구현 문제
- 별도의 탐색이나 정렬 없이 나눗셈과 나머지 연산만으로 층/호실을 결정할 수 있음

#### 복잡도
- 시간복잡도: `O(T)` — 테스트케이스 수만큼 반복

#### 아쉬운 점 / 개선할 사항
- `N % H`와 `N // H`를 각각 두 번 호출하고 있어 가독성이 낮다
- 이를 개선하기 위해 `divmod(N, H)`로 몫과 나머지를 한 번에 구할 수 있다
  - `(a // b, a % b)`를 튜플로 한 번에 반환함
- 호실 번호의 두 자리 포맷팅을 if문으로 처리했는데 `str(room_num).zfill(2)`로 간결하게 표현할 수 있었다
  - `zfill(n)` = `rjust(n, '0') : 0으로 오른쪽 정렬. 0을 앞에서부터 채운다
  - `ljust(n, char)` : 왼쪽 정렬. 오른쪽부터 원하는 문자로 채운다
  - `center(n, char)` : 가운데 정렬. 양쪽을 지정 문자로 채운다 
...
for _ in range(T):
    ...
    q, r = divmod(N, H)
    floor = r if r != 0 else H
    room_num = q + 1 if r != 0 else q
    print(f"{floor}{str(room_num).zfill(2)}")
```
