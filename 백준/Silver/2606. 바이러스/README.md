...
### ✅ Solve log
#### 아쉬운 점 / 개선할 사안
- DFS를 사용하여 풀이를 했는데, 재귀 함수에서 `cnt`를 누적하는 방식보다 모든 탐색을 끝낸 뒤 `visited`의 True 개수를 세는 방식이 더 단순하고 안정적이었다. 이 경우 문제 요구상 시작 노드인 1번은 제외해야 하므로 `-1`을 한다.
```python
dfs(1)
print(sum(visited) - 1)
```
...
- 시간 : O(N+E)
- 공간 : O(N+E)
- 여기서 N은 노드 개수 = 컴퓨터 수이고, E는 간선 수 = 연결된 pair 수다.
