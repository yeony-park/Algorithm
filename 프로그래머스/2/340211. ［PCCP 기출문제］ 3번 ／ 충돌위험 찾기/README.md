...
### ✅ solve log
#### 아이디어
- 각 로봇이 시간마다 어떤 위치(좌표)에 있는지 시뮬레이션해서 기록한다
- 두 지점 사이의 이동이 최단거리로 고정되기 때문에 `r`좌표를 먼저 맞춘 뒤 `c`좌표를 이동한다
- 같은 시간에 같은 좌표에 2대 이상 모든 경우만 위험 상황으로 카운트한다

#### 아쉬운 점/개선할 점
- 처음에 BFS나 최단경로 탐색 문제처럼 생각했는데, 실제로는 경로가 규칙으로 이미 정해져 있어서 시뮬레이션으로 바로 처리할 수 있다는 걸 늦게 깨달았다
- 충돌 여부만 판단하지 않고 매 초마다 좌표별 로봇 수를 세는 문제로 단순화하는 연습이 필요할 것 같다

#### 배운 점
- 최단경로라고 항상 BFS를 써야하는 건 아니고, 이동 규칙이 확실할 때는 직접 시뮬레이션하는 게 더 단순하다
- 시간대별 상태를 관리하는 문제에서는 `시간 -> 좌표 카운트` 구조로 생각하면 구현이 깔끔해짐
