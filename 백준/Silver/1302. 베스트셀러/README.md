...
### 다시 풀기 (복습)
#### 1차 풀이: 베스트셀러.py
- 정렬 기준을 key=lambda x: (-빈도, 제목) 형태로 한 번에 처리하는 방식에 익숙해지기 위해 참고 코드를 기반으로 작성하였다.
- sorted(items, key=lambda x: (-x[1], x[0]))로 정렬하여 해결

#### 복습 풀이: 베스트셀러-re.py
- 동일한 로직을 카운팅 → 최댓값(mx) 추출 → 후보 필터링 → 사전순으로 구현하였다.
- lambda 기반 정렬이 익숙하지가 않아서 로직을 단계적으로 분리하여 구현하였다.
- mx로 최대 빈도만 필터링한 뒤 제목만 정렬하여 첫 값 선택
- sys.stdin.readline과 strip()을 사용하여 입력 처리 안정성 개선

#### 다음 목표
- sorted(..., key=...) 패턴을 더 풀어보면서 lambda 활용에 더 익숙해지기
