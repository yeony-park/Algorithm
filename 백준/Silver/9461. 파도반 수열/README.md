...
### ✅ solve log
#### 아이디어
- 재귀 + 메모이제이션 방식으로 파도반 수열을 구현
- 중복 계산을 피하기 위해 한 번 구한 값은 DP 배열에 저장해서 재사용

#### 아쉬운 점/ 개선할 점
- 인덱스를 `i-1`로 한 칸 밀어서 처리하다 보니 점화식과 기저조건을 더 직관적으로 작성하지 못한 점이 아쉬움
- 다음에는 `d[i]` 자체를 그대로 사용하는 방식으로 구현해서 가독성을 높여봐야겠다
