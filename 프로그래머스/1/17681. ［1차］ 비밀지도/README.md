...
### ✅ solve log
#### 아이디어
- arr1[i]와 arr2[i]를 각각 2진수 문자열로 바꾼 뒤, 길이를 n으로 맞춰 한 글자씩 비교했다.
- 같은 위치에서 둘 중 하나라도 1이면 벽이므로 #, 둘 다 0이면 공백 ' '을 넣었다.
- 각 행을 완성해서 result 리스트에 저장한 뒤 반환했다.

#### 아쉬운 점/개선할 점
- 처음에 비트 연산자를 떠올렸지만 정확히 구현하지 못해서 문자열 접근 방식으로 우회했다
- 두 수를 `OR(|)`연산하면 해당 위치 중 하나라도 1인 비트가 남기 때문에 비트 연산이 더 적합하다.
- `merged = arr[i] | arr2[i]`로 비트단위로 합치기
...
def solution(n, arr1, arr2):
    ...
    return answer
```
- `zfill()`은 우측정렬로 0으로만 채우는 전용메소드
- `rjist()`는 오른쪽 정렬을 하면서 왼쪽을 원하는 문자로 채움
    - 사용 예시 : `'101'.rjust(5, '0')` -> '00101'
- 비트연산자로 계산해도 10진수로 결과값이 나오기 때문에 `bin()`으로 2진수 문자열로 만들어 줘야 함
